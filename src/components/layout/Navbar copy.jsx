...
export default function Navbar() {
  ...
  const onDropdownClick =
    (groupId) =>
      ({ key }) => {
        ...
        // account dropdown items should open modal instead of navigate
        if (groupId === "account" && (key === "signin" || key === "signup")) {
          setAuthModalType(key);
  ...
  return (
    <nav ref={navRef} className="w-full border-b border-slate-200 bg-white">
      ...
      <Modal
        title={authModalType === "signin" ? "Sign In" : "Sign Up"}
        open={authModalVisible}
        onCancel={() => setAuthModalVisible(false)}
        footer={null}
      >
        {/* placeholder content; swap in real form components as needed */}
        {authModalType === "signin" ? (
          <>
            <p>Please implement a sign‑in form here.</p>
          ...
          <>
            <p>Please implement a sign‑up form here.</p>
            <div className="mt-4 text-right">
              <Link
                to="/signup"
                onClick={() => setAuthModalVisible(false)}
